<script lang="ts">
	...
	onMount(() => {
		if (browser) {
			...
			if (listCheck != null) lists = JSON.parse(listCheck);
		}
		showMain = true;
	});
	...
	function addList() {
		...
		if (browser) localStorage.setItem("lists", JSON.stringify(lists));
		inputName = "";
		inputDes = "";
	}
...
<header class="mb-4 flex items-center">
	<h1 class="w-fit border-b-2 border-LightGray/50 px-4">Lists</h1>
	<div class="flex-grow" />
...
{#if showMain}
	{#if lists.length != 0}
		<section class="grid gap-4 px-4 max-sm:grid-cols-1 sm:grid-cols-4">
			{#each lists as list, i}
				<article
					class="rounded-lg px-4 py-2 shadow-lg transition-shadow duration-300 ease-in hover:shadow-LightBlue">
					<div class="flex items-center space-x-2 border-b border-[#424651]/50 py-1">
						<h2 class="w-full text-ellipsis">{list.name}</h2>
						<section class="flex space-x-2">
							<a
								href="Lists/{i}"
								class="w-fit rounded-full p-1 transition-all duration-200 ease-in hover:text-LightBlue">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									fill="none"
									viewBox="0 0 24 24"
							...
							<button
								...
								class="w-fit rounded-full p-1 transition-all duration-200 ease-in hover:text-red-500">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									fill="none"
									viewBox="0 0 24 24"
	...
	{:else}
		<p class="px-4 text-center text-lg font-medium tracking-wide">
			It looks like you don't have any lists. Click the "<span class="text-LightBlue">+</span>"
			button to add one.
		</p>
	{/if}
{/if}
...
<PopUp bind:showPop={showDelPop} bind:this={delPop} title="Confirm Delete" showXBtn={false}>
	<section class="px-4 py-1">
		<p class="indent-4">
			Woah there. Are you sure you really want to delete this list before completing it?
		</p>
		<p>Note: Once lists are deleted, they can <span class="text-red-500">NEVER</span> come back</p>
	</section>
...
<PopUp bind:showPop={showAddPop} bind:this={addPop} title="New List" showXBtn={true}>
	<section class="w-full flex-row space-y-2 px-4 py-1">
		<input type="text" placeholder="List name" bind:value={inputName} />
		<input type="text" placeholder="A short description" bind:value={inputDes} />
